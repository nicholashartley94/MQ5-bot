...
void OnTick()
{
    ...
    MqlRates rates[];
    int copied = CopyRates("USDJPY", PERIOD_M1, 0, numberOfCandles, rates);
...
{
    double ma[];
    int maHandle = iMA("USDJPY", PERIOD_M1, period, 0, MODE_SMA, PRICE_CLOSE);
    if (maHandle < 0)
    {
...
void HedgePosition()
{
    ...
    if (PositionSelect("USDJPY"))
    {
        ...
        {
            if (trade.Sell(hedgeVolume, "USDJPY"))
            {
        ...
        {
            if (trade.Buy(hedgeVolume, "USDJPY"))
            {
