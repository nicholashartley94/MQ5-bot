...
{
    double ma[];
    int maHandle = iMA("EURJPY", PERIOD_M1, period, 0, MODE_SMA, PRICE_CLOSE);
    if (maHandle < 0)
    {
...
void Bot(const MqlRates &rates[])
{
    ...
    double bid = SymbolInfoDouble("EURJPY", SYMBOL_BID);
    double ask = SymbolInfoDouble("EURJPY", SYMBOL_ASK);
...
void HedgePosition()
{
    ...
    if (PositionSelect("EURJPY"))
    {
        ...
        {
            if (trade.Sell(hedgeVolume, "EURJPY"))
            {
        ...
        {
            if (trade.Buy(hedgeVolume, "EURJPY"))
            {
