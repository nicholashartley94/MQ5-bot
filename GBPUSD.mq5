...
{
    double ma[];
    int maHandle = iMA("GBPUSD", PERIOD_M1, period, 0, MODE_SMA, PRICE_CLOSE);
    if (maHandle < 0)
    {
...
void HedgePosition()
{
    ...
    if (PositionSelect("GBPUSD"))
    {
        ...
        {
            if (trade.Sell(hedgeVolume, "GBPUSD"))
            {
        ...
        {
            if (trade.Buy(hedgeVolume, "GBPUSD"))
            {
